...
- **หน่วยความจำช้าและอันตราย** การเขียน memory ลงโปรเจ็คกลาง (AgentsRoom) ใช้เวลา 3–10 วินาที (บางครั้งเกินสองนาที) และ agent ที่เขียนโดยไม่อ่านก่อนก็เขียนทับโน้ตทั้งฉบับ ผลคือ agent เลิกเขียน — แล้ว session ก็ถูก compact ทั้งที่สิ่งที่ค้นพบยังอยู่แค่ในแชต — หรือไม่ก็เขียนแล้วทำพัง
...
- **plugin เดียว อัปเดตตัวเอง** `blm init` ติดตั้งไบนารี plugin ของ Claude Code และ hook; `blm self-update` อัปเดตทั้งคู่ CLI กับ MCP ใช้โค้ดชุดเดียวกัน สิ่งที่เจ้าของเห็นในเทอร์มินัลคือสิ่งที่ agent เห็น
...
| flag | store | sync |
| --- | --- | --- |
| *(ไม่ใส่)* | `.claude/blm` | ไม่มี — ไฟล์คือความจริง (`blm_sync` ไม่ถูกประกาศ) |
