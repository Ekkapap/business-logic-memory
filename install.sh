...
# ดาวน์โหลด binary จาก GitHub Releases → ~/.blm/bin/blm (+ symlink ~/.local/bin/blm) → เติม PATH → blm init <args ที่ส่งมา>
set -e
...
# ที่อยู่มาตรฐาน: ~/.blm/bin/blm (blm self-update แทนไฟล์นี้) · ~/.local/bin/blm = symlink มาที่นั่น (PATH)
BLM_HOME="${BLM_HOME:-$HOME/.blm}"; BIN_DIR="$BLM_HOME/bin"; LINK_DIR="${BLM_BIN_DIR:-$HOME/.local/bin}"
...
ln -sfn "$BIN_DIR/blm" "$LINK_DIR/blm"
echo "blm: installed $BIN_DIR/blm ($LINK_DIR/blm → it)"
"$LINK_DIR/blm" path
"$LINK_DIR/blm" init "$@"
