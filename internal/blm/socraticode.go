...
func buildFromSocraticode(root string, fg scFileGraph, files map[string]*scFilePayload) Graph {
	...
	for rel, f := range files {
		...
		for _, c := range f.OutgoingCalls {
			...
			// call ไปไฟล์ที่ import อยู่แล้ว → edge เดิมมีอยู่ก่อน (kind import) ยกระดับเป็น call และนับ (ไม่งั้น calls เป็น 0 ทั้งกราฟ เห็นบนโปรเจ็คจริง 2026-09-09: 2,869 call entries นับได้ 0)
			k := filepath.ToSlash(rel) + "->" + filepath.ToSlash(target)
			if e, ok := edgeSet[k]; ok && kind == "call" {
				if e.Kind != "call" {
					e.Kind = "call"
					edgeSet[k] = e
				}
