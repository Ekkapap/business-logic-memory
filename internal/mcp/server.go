...
func (s *Server) Call(name string, a map[string]any) (any, error) {
	...
	case "blm_conflict":
		if act, _ := a["action"].(string); act == "mark" {
			r := s.store.Mark()
			return map[string]any{"ok": true, "mark": r, "next": "blm.md and the notes now carry [Conflict](<report>) for every waiting report; resolve removes them"}, nil
		}
	...
	case "blm_conflicts":
		...
		if id, ok := a["id"].(float64); ok && id > 0 {
			for _, c := range list {
				if c.ID == int(id) && c.Status == "wait" {
					t, err := s.store.RenderConflict(c)
					if err != nil {
						return nil, err
					}
					return map[string]any{"conflict": c, "terminal": t}, nil
				}
			}
