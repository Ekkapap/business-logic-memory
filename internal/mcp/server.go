...
func tools(c blm.Config) []tool {
	...
	all := []tool{
		...
		{"blm_conflict", "Agent cannot decide a blm_diff conflict and the owner is not available: file it. The report is written for the OWNER — write `reason` in the owner's language (Thai on this project) saying what each side claims. Writes one report per conflicting region to <store>/conflicts/<id>-<topic>-<heading>.wait.md (block A = cloud, block B = draft, each with a markdown checkbox; the owner may edit a block before ticking it), keeps the 3-way result for reassembly, and tags the subtopic heading in the draft with [Conflict: #1, #2]. The draft stays unpushed until blm_resolve.",
		...
		{"blm_conflicts", "List conflict reports: status wait/done (from the file name), topic › subtopic, which block is ticked. Scan this to see what still needs the owner.", obj(map[string]any{})},
...
func (s *Server) Call(name string, a map[string]any) (any, error) {
	...
	case "blm_conflicts":
		return map[string]any{"conflicts": s.store.ListConflicts()}, nil
	case "blm_resolve":
