...
func tools(c blm.Config) []tool {
	...
	all := []tool{
		...
		{"blm_conflict", "Agent cannot decide a blm_diff conflict and the owner is not available: file it. Writes one report per conflicting region to <store>/conflicts/<id>-<topic>-<heading>.wait.md (block A = cloud, block B = draft, each with a markdown checkbox; the owner may edit a block before ticking it), keeps the 3-way result for reassembly, and tags the subtopic heading in the draft with [Conflict: #1, #2]. The draft stays unpushed until blm_resolve.",
